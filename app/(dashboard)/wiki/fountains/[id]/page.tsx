...
import { getWikiFountainDetails, getWikiFountainList } from '@/api/wikipedia';
...
export default function FountainsPage() {
  ...
  const { data, isLoading, error } = useWikipediaModelDetail(getWikiFountainDetails, id);
  const { objects, isLoadingObjects, errorObjects } = useWikipediaModelList(getWikiFountainList);
  ...
  return (
    ...
    <WikiObjectDetailBoxes 
      ...
      isLoading={isLoading}
      error={error}
      titleInfoBox='Informationen zum Brunnen'
      titleImageBox='Bilder des Brunnen'
      titleNearbyObjectBox='Brunnen in der Umgebung'
      titleLocationBox='Verortung des Brunnen'
    />
    <WikiObjectList
        objects={objects}
        isLoading={isLoadingObjects}
        error={errorObjects}
        titleListBox='Liste weiterer Brunnen'
    />
    </div>
  );
}
